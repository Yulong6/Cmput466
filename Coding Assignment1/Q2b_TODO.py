...
def train(X_train, y_train, X_val, y_val,decay):
    ...
    for epoch in range(MaxIter):
        ...
        for b in range( int(np.ceil(N_train/batch_size)) ):
            ...
            p = np.dot(X_batch.T, (y_hat_batch - y_batch)) + decay * w
...
for i in decay:
    ...
    if min(losses_train) < 100:
        decay_best = i
losses_train, risks_val, epoch_best, risk_best, w_best = train(X_train, y_train, X_val, y_val,decay_best)
...
print("The best hyperparameter weight decay is :", decay_best)
